...
CGI::CGI(std::string script, std::string query, std::string addr, std::string port,
    ...
    : script(script), query(query), addr(addr), port(port), server_name(name), server_port(srv_port) {}

void CGI::execute() {
    std::string interpreter;
    if (script.substr(script.find_last_of(".") + 1) == "php") {
        interpreter = "php";
    } else if (script.substr(script.find_last_of(".") + 1) == "py") {
        interpreter = "python3";
    } else {
    ...
    char* const argv[] = { const_cast<char*>(interpreter.c_str()), const_cast<char*>(script.c_str()), NULL };

    pid_t pid = fork();
    if (pid == 0) {
        if (execvp(interpreter.c_str(), argv) == -1) {
            std::cerr << "Erreur lors de l'exécution de " << interpreter << " pour le script " << script << std::endl;
            exit(1);
        }
    } else if (pid < 0) {
