...
void    BlocLocation::_handleUploadPath(std::vector<std::string> tokens)
{
    ...
    if (access((_root_path + _location_path + tokens[1]).c_str(), F_OK) == 0 && !isDirectory(_root_path + _location_path + tokens[1]))
        Logger::log(Logger::ERROR, "invalid config file. => invalid path \"" + tokens[1] + "\"");
...
void    BlocLocation::_handleCGIExt(std::vector<std::string> tokens)
{
    ...
    if (tokens[1] == ".php")
        ...
        Logger::log(Logger::ERROR, "invalid config file. => unsupported cgi " + tokens[1]);
}
...
void    BlocLocation::_handleCGIPass(std::vector<std::string> tokens)
{
    ...
    if (isDirectory(tokens[1]))
        Logger::log(Logger::ERROR, "invalid config file. => path is a directory: " + tokens[1]);
    
    if (!isExecutable(tokens[1]))
        Logger::log(Logger::ERROR, "invalid config file. => not executable: " + tokens[1]);
...
void    BlocLocation::_handleRedirect(std::vector<std::string> tokens)
{
    ...
    if (code < 300 || code > 307 || code == 304)
        Logger::log(Logger::ERROR, "invalid config file. => invalid code " + tokens[1]);
        ...
        && !(tokens[2].rfind("https://", 0) == 0))
    {
        ...
        if (!isReadable(url))
            Logger::log(Logger::ERROR, "invalid config file. => not readable: " + url);
